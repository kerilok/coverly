...
export function AuthNav() {
  ...
  useEffect(() => {
    ...
    if (!supabase) {
      return () => window.removeEventListener("coverly-auth", syncLocal);
    }
  ...
  return (
    <div className="flex items-center gap-2">
      <Link href="/account" title={name} className="grid h-11 w-11 place-items-center rounded-full bg-gradient-to-br from-plum to-pink-400 text-xs font-bold text-white">
      ...
      <button
        ...
        className="grid h-11 w-11 place-items-center rounded-xl border line"
      >
        <LogOut size={16} />
      </button>
    </div>
